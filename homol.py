...
if len(res_report.json()) != 1: # Verificar se há dados de cotas
    ...
    with col1:
        ...
        st.metric('Rentabilidade - 12 meses e Início', value = f'{rent_12m:.0%}', delta = f'{ult_rent:.0%}')
    with col2:
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=df_cota_fundo.index, y=df_cota_fundo['rent_norm']))
        fig.update_layout(title_text = '<b>Histórico Rentabilidade</b>', yaxis_tickformat = '%', width=800, height=400,

...
def Carteira():
    ...
    if st.button('Calcular'):
        ...
        for i in portifolio['Ação']:
            ...
            portifolio['Ult. Fechamento'][count] = yf.download(i + '.SA', period='1d')['Adj Close'][
                0]  # Pegar o ultimo preço de fechamento da lista
            count += 1
        ...
        preco_bova11 = inv.get_etf_recent_data('Ishares Ibovespa', country='brazil').tail(1)['Close'][
            -1]  # Pegar o ultimo preço de fechamento da lista
        preco_winfut = inv.get_index_recent_data('Bovespa', country='brazil')['Close'][
            -1]  # Pegar o ultimo preço de fechamento da lista
        ...
        portifolio = portifolio.style.format(
            {"Ult. Fechamento": "R${:20,.2f}", "Valor": "R${:20,.2f}", "%": "{:.0%}", "Beta": "{:.2}",
             "Beta Pond": "{:.2}"})
        ...
        '** **'
        sns.heatmap(retornos.tail(255).corr(), annot=True,
                    annot_kws={"size": 8});  # HeatMap da correlação entre os Ativos do periodo de 1 ano
...
def Correlacao():
    ...
    if st.button('Calcular'):
        with st.spinner('Baixando Histórico e Calculando...'):
            ...
            retornos.rename(columns={'^BVSP': 'Indice Bovespa', 'USDBRL=X': 'Indice Dolar', '^GSPC': 'Indice SP500',
                                     '^DJI': 'Indice Dow Jones', '^IXIC': 'Indice NASDAQ'}, inplace=True)
            count = 0
            for item in retornos.columns:
                if 'Indice' not in item:
                    string_a = retornos.columns[count]
            ...
            for i in range(len(tickers) - 1):
                ...
                if count != 1: correlacao_composicao = pd.merge(correlacao_composicao, correlacao_ativo, how='inner',
                                                                on='Date')
                count += 1
            ...
            fig = correlacao_composicao.iplot(asFigure=True, xTitle='Data', yTitle='Correlação',
                                              title='Correlação entre ' + selecao[0] + ' e os Ativos listados')
            st.plotly_chart(fig)

            'HeatMap da correlação entre os ativos nos últimos **12 meses**:'
            # plt.subplots(figsize=(18,10))
            sns.heatmap(retornos.tail(255).corr(),
                        annot=True);  # HeatMap da correlação entre os Ativos do periodo de 1 ano
            st.pyplot()
...
def Quedas():
    ...
    if pressed_calc:
        ...
        dados_df = dados_df.style.applymap(_color_red_or_green,
                                           subset=['% Queda', '% Abert. Seguinte', '% Fech. Seguinte',
                                                   '% Var. dia Seguinte']).format(
            {"% Queda": "{:.2%}", "% Abert. Seguinte": "{:.2%}", "% Fech. Seguinte": "{:.2%}",
...
def Distancia_Medias():
    ...
    if st.button('Calcular'):
        ...
        fig = df.iplot(asFigure=True, xTitle='Data', yTitle='Preço', dimensions=(900, 600),
                       title=ticker + ' - Distância em relação a SMA ' + str(SMA) + '- Threshold: ' + str(threshold),
                       hline=[-threshold, 0, threshold])
        st.plotly_chart(fig)
...
def Retornos_mensais():
    ...
    if st.button('Calcular'):
        ...
        # Dados do Investing
        if pais == 'Brasil' and opcao == 'Ações':
            papel = inv.get_stock_historical_data(ticker, country='brazil', from_date=data_inicial, to_date=data_final,
                                                  interval='Monthly')['Close'].pct_change(1)
        if pais == 'Brasil' and opcao == 'Indices':
            papel = inv.get_index_historical_data(ticker, country='brazil', from_date=data_inicial, to_date=data_final,
                                                  interval='Monthly')['Close'].pct_change(1)
        if pais == 'Estados Unidos' and opcao == 'Ações':
            papel = \
            inv.get_stock_historical_data(ticker, country='united states', from_date=data_inicial, to_date=data_final,
                                          interval='Monthly')['Close'].pct_change(1)
        if pais == 'Estados Unidos' and opcao == 'Indices':
            papel = \
            inv.get_index_historical_data(ticker, country='united states', from_date=data_inicial, to_date=data_final,
                                          interval='Monthly')['Close'].pct_change(1)
...
def Op_Calc_Prob():
    ...
    ticker = st.selectbox('Escolha a Ação (Clique no campo e digite as iniciais do Ticker)', stocks_df)
    stock_price = inv.get_stock_recent_data(ticker, country='brazil')['Close'][
        -1]  # Pegar o ultimo preço de fechamento da lista
    # st.write('Preço do Ativo: ', stock_price)
    stock_price = st.number_input('Preço do Ativo (Preço com 15 min de atraso. Atualize com o preço atual)',
                                  stock_price)
...
def Setorial():
    ...
    if st.button('Calcular'):
        ...
        st.table(portifolio.style.format(
            {"Ult. Fechamento": "R${:20,.2f}", "Valor": "R${:20,.2f}", "%": "{:.0%}", "Beta": "{:.2}",
             "Beta Pond": "{:.2}"}))
...
'Análise do Beta da Carteira', 'Análise Setorial', 'Correlação entre Ativos', 'Análise de Quedas / Dia Seguinte',
'Análise de Distância das Médias', 'Análise de Retornos Mensais', 'Opções - Calculadora de Probabilidade'))
...
indices = [{'Ticker': 'Indice Bovespa'}, {'Ticker': 'Indice Dolar'}, {'Ticker': 'Indice SP500'},
           {'Ticker': 'Indice Dow Jones'}, {'Ticker': 'Indice NASDAQ'}]
